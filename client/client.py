...
def connect_to_server(server_host, port, token):
    ...
    try:
        ...
        hostname = get_hostname()
        send_message(sock, f"{MSG_REGISTER}:{hostname}:{token}")

        # Wait briefly for rejection or success
        sock.settimeout(2)  # 2 second timeout for auth response
        try:
            response = recv_message(sock)
            if response and response.startswith(MSG_TOKEN_STATUS):
                ...
                if len(status_parts) > 1:
                    ...
                    if status == TOKEN_REVOKED:
                        ...
                        sock.close()
                        return "REVOKED_TOKEN"  # Special return value for revoked
                    elif status == TOKEN_INVALID:
                        print(f"[!] Authentication failed - token is invalid")
                        sock.close()
                        return "INVALID_TOKEN"  # Special return value for invalid
        except socket.timeout:
            # No rejection message = success
            pass
